...
energy_valence <- ggplot(all_music, 
       aes(energy, 
           valence, 
           color = loudness)) + # speechiness, acousticness, liveness?
...
# Simple boxplot tempo
ggplot(all_music, aes(y = category,
                      x = tempo)) +
  geom_path(alpha = 0.3, 
               fill = "grey20", 
               color = "grey20", 
...
# Attempt area plot tempo vs ...

all_music %>% ggplot(aes(x = tempo, 
                         y = danceability, 
                         color = category)) +
  geom_jitter() + 
  geom_smooth()
